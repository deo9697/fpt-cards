-- Reuse eligible snapshots and mapping flags across latest/low/history reads.
-- Preserve the distinct active and derived eligibility rules, including manual mappings.
...
begin
  ...
  with sources as (
    ...
    from market_provider_printings
    ...
    from market_price_snapshots s join monitored m on m.printing_id=s.printing_id
    ...
    order by printing_id,market_reference_type(provider,price_type),captured_at desc
  ), history_7d as (
    select distinct on (printing_id) printing_id,normalized_price
