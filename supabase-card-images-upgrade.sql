-- Salvataggio miniature catalogo. Eseguire dopo supabase-batch-loans-upgrade.sql.
alter table public.loans add column if not exists card_external_id bigint;
alter table public.loans add column if not exists card_image text;

create or replace function public.create_team_loans(p_token text, p_cards jsonb, p_borrower_slug text, p_notes text default '')
...
begin
  ...
  return query insert into public.loans(card_name, quantity, owner_slug, borrower_slug, notes, card_external_id, card_image)
    select trim(c->>'name'), (c->>'quantity')::integer, me, p_borrower_slug, left(coalesce(p_notes,''),500), nullif(c->>'externalId','')::bigint, nullif(left(c->>'image',500),'')
