...
returns jsonb language plpgsql security definer set search_path = public, extensions as $$
declare me text := public.session_member(p_token); total integer; level integer; used_today integer;
begin
  if me is null then raise exception 'Sessione scaduta'; end if;
  insert into public.member_progression(member_slug, total_xp, level) values (me, 0, 1)
    on conflict (member_slug) do nothing;
  select total_xp, level into total, level from public.member_progression where member_slug = me;
  select coalesce(sum(xp_amount), 0) into used_today from public.xp_transactions
    where member_slug = me and source_type = 'match' and created_at::date = current_date;
  return jsonb_build_object('totalXp', total, 'level', level, 'xpToday', used_today, 'dailyCap', 100);
end;
$$;
