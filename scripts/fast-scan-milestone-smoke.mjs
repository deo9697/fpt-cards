...
const advanced=[];const focusTrack={getCapabilities:()=>({focusMode:['continuous','single-shot'],zoom:{min:1,max:3,step:.1},torch:true}),getSettings:()=>({deviceId:'rear-focus',focusMode:'continuous',zoom:1.35}),applyConstraints:async value=>advanced.push(value),stop:()=>{}};const focusStream={getTracks:()=>[focusTrack],getVideoTracks:()=>[focusTrack]};const focusVideo={srcObject:null,muted:false,playsInline:false,play:async()=>{}};const focusCamera=new FastScanCamera({getUserMedia:async()=>focusStream,enumerateDevices:async()=>[]});await focusCamera.start(focusVideo);assert.equal(focusCamera.focusSupported,true);assert.equal(focusCamera.zoomSupported,true);assert(advanced.some(item=>item.advanced[0].focusMode==='continuous'),'autofocus continuo non applicato');assert(advanced.some(item=>item.advanced[0].zoom===1.35),'zoom moderato non applicato');assert.equal(await focusCamera.refocus(),true);assert(advanced.some(item=>item.advanced[0].focusMode==='single-shot'),'tap/refocus non usa single-shot');focusCamera.stop();
...
for(const [name,dark,light] of [['poca luce',8,62],['luce forte',190,255],['sleeve lucida',30,250]]){const width=36,height=14,data=new Uint8ClampedArray(width*height*4);for(let p=0;p<width*height;p++){const value=p%9<3?dark:light;data[p*4]=data[p*4+1]=data[p*4+2]=value;data[p*4+3]=255;}const image={data};const ctx={getImageData:()=>image,putImageData:()=>{}};const stats=preprocessCodeImage(ctx,width,height);assert.equal(stats.mode,'adaptive',`${name}: preprocessing non adattivo`);assert([...data].filter((_,index)=>index%4===0).every(value=>value===0||value===255),`${name}: threshold non binario`);}
