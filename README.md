...
Il pannello debug include un benchmark sperimentale tra Tesseract (Engine A, ancora unico motore production) e PaddleOCR.js `0.4.2` con i modelli `PP-OCRv6_tiny` (Engine B). Dopo una scansione, inserire il codice reale nel campo DEV e avviare il confronto: entrambi i motori ricevono lo stesso canvas grayscale, mentre il report mostra testo, confidence, latenza, distanza dal codice reale e ranking sul catalogo locale (`exact`, `near`, `ambiguous`, `reject`). I report, senza immagini, restano soltanto in memoria in `window.__fastScanEngineBenchmarks`.

Engine B viene importato da CDN esclusivamente alla pressione del pulsante. Usa ONNX Runtime Web in WASM SIMD single-thread per mantenere la compatibilità con Android Chrome e Safari/iOS, senza dipendere da WebGPU. Il cold payload non compresso stimato è circa 30,5 MB: OpenCV.js ~11,8 MB, ONNX WASM ~12,4 MB e modelli tiny ~6,3 MB, oltre al glue code. Per questo resta fuori dal bundle iniziale e viene rilasciato dopo ogni benchmark. `performance.memory` è riportato quando il browser lo espone (tipicamente Chromium); su Safari il valore è indicato come non disponibile.
