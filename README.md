...
### Milestone 3.2 · OCR Engine Benchmark

Il pannello debug include un benchmark tra Tesseract (Engine A/fallback) e PaddleOCR.js `0.4.2` con i modelli `PP-OCRv6_tiny` (Engine B/candidato principale). Dopo una scansione, inserire il codice reale nel campo DEV e avviare il confronto: entrambi i motori ricevono lo stesso canvas grayscale, mentre il report mostra testo, confidence, latenza, zoom, distanza dal codice reale e ranking sul catalogo locale (`exact`, `near`, `ambiguous`, `reject`). I report non contengono immagini e vengono conservati localmente in `fpt-fast-scan-ocr-benchmarks-v1` (massimo 50) e resi disponibili in `window.__fastScanEngineBenchmarks`, così è possibile completare il campione di 20–30 carte anche tra più aperture della PWA.

PaddleOCR non viene caricato all'avvio generale dell'app. Entrando nel Fast Scan viene inizializzato dopo l'avvio della fotocamera: la UI mostra `Preparazione Fast Scan`, poi `Scanner pronto`. La stessa sessione ONNX viene riutilizzata per tutte le letture della sessione; Tesseract viene caricato soltanto se Paddle non è disponibile o genera un errore. Usa ONNX Runtime Web in WASM SIMD single-thread per mantenere la compatibilità con Android Chrome e Safari/iOS, senza dipendere da WebGPU. Il cold payload non compresso stimato è circa 30,5 MB: OpenCV.js ~11,8 MB, ONNX WASM ~12,4 MB e modelli tiny ~6,3 MB, oltre al glue code.

Gli asset non fanno parte della precache della shell. La service worker li salva on demand nella cache runtime versionata `fpt-cards-paddle-v1`, includendo dipendenze jsDelivr e modelli ufficiali `paddle-model-ecology.bj.bcebos.com`; quando disponibile viene inoltre richiesta la persistenza dello storage del browser. Il self-hosting completo è stato rinviato finché Paddle resta candidato: aggiungerebbe circa 30,5 MB al deploy Vercel/repository senza migliorare il primo download. Versioni e URL sono comunque bloccati esplicitamente nel codice, mentre la cache rende indipendenti dal CDN le aperture successive già scaricate. `performance.memory` è riportato quando il browser lo espone (tipicamente Chromium); su Safari il valore è indicato come non disponibile.
