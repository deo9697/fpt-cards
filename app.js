...
function newLoanView() {
  ...
  return `<div class="section-heading"><div><span class="eyebrow">Banco scambi</span><h2>Registra un prestito</h2><p>${game.name}</p></div><span class="game-pill ${state.game}">${game.mark}</span></div><div class="card exchange-form"><form id="loan-form"><label for="card-name">Cerca nel catalogo ${game.short}</label><div class="catalog-search"><input id="card-name" autocomplete="off" placeholder="${hint}"><div id="card-suggestions" class="suggestions"></div></div>
...
function loansView() {
  ...
  return `<div class="section-heading"><div><span class="eyebrow">Registro del team</span><h2>Archivio prestiti</h2><p>${GAMES[state.game].name}</p></div><span class="total-pill">${base.length} totali</span></div>
...
function teamView() {
  ...
  return `<div class="section-heading"><div><span class="eyebrow">Cerchia dei duellanti</span><h2>Il team</h2><p>${MEMBERS.length} membri nel registro F.P.T</p></div><span class="team-seal">FPT</span></div><section class="card notification-setting"><div><strong>Notifiche richieste</strong><small>${notificationState}</small></div><button class="btn secondary small" id="enable-notifications">${configured ? 'Riconfigura' : 'Attiva'}</button></section>${manager}<div class="team-list">${rows}</div>`;
}
