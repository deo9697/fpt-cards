export class FastScanCamera {
  ...
  async captureSnapshot(roiElement,{preferVideoFrame=false,includeAlternate=false}={}) {
    ...
    let source=video,sourceType='video-canvas',bitmap=null,alternateCanvas=null,grabMs=0,completed=false;
...
function releaseSnapshot(snapshot){const canvas=snapshot?.canvas;if(canvas){clearCanvas(canvas);snapshot.canvas=null;}for(const alternate of snapshot?.alternates||[])clearCanvas(alternate?.canvas);snapshot.alternates=[];snapshot.roi=null;snapshot.release=()=>{};}
