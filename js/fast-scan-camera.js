export class FastScanCamera {
  ...
  async captureSnapshot(roiElement,{preferVideoFrame=false,includeAlternate=false,debugComparative=false}={}) {
    ...
    let source=video,sourceType='video-canvas',bitmap=null,rawCanvas=null,alternateCanvas=null,grabMs=0,completed=false;
    ...
    try{
      ...
      const drawStarted=performance.now();this.snapshotCanvas.width=width;this.snapshotCanvas.height=height;const ctx=this.snapshotCanvas.getContext('2d',{willReadFrequently:true});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.drawImage(source,crop.sx,crop.sy,crop.sw,crop.sh,0,0,width,height);const frameCapturedAt=performance.now(),frameCapturedWallAt=new Date().toISOString();if(includeAlternate||debugComparative){rawCanvas=cloneCanvas(this.snapshotCanvas);if(includeAlternate)alternateCanvas=cloneCanvas(rawCanvas);}const drawMs=performance.now()-drawStarted;
      const preprocessStarted=performance.now(),preprocessing=preprocessCodeImage(ctx,width,height,{mode}),alternates=[];if(alternateCanvas){const alternateMode=mode==='adaptive'?'grayscale':'adaptive',alternatePreprocessing=preprocessCodeImage(alternateCanvas.getContext('2d',{willReadFrequently:true}),width,height,{mode:alternateMode});alternates.push({canvas:alternateCanvas,preprocessing:alternatePreprocessing});}const preprocessMs=performance.now()-preprocessStarted,debugQuality=debugComparative?{raw:preprocessing.sharpness,[mode]:measureCanvasSharpness(this.snapshotCanvas),...(alternates[0]?{[alternates[0].preprocessing.mode]:measureCanvasSharpness(alternates[0].canvas)}:{})}:null,snapshot={canvas:this.snapshotCanvas,rawCanvas,previewRoi,roi:crop,preprocessing,alternates,mapping:{...mapping,snapshotCrop:crop,sourceWidth,sourceHeight},debugQuality,source:sourceType,resolution:{width:sourceWidth,height:sourceHeight},frameCapturedAt,frameCapturedWallAt,timing:{totalMs:performance.now()-started,grabMs,cropMs,drawMs,preprocessMs}};
      snapshot.release=()=>releaseSnapshot(snapshot);completed=true;return snapshot;
    }finally{if(!completed){clearCanvas(this.snapshotCanvas);clearCanvas(rawCanvas);clearCanvas(alternateCanvas);}bitmap?.close?.();bitmap=null;rawCanvas=null;alternateCanvas=null;source=null;}
  }
...
export function sourceCropDetails(video,roiElement,sourceWidth=video.videoWidth,sourceHeight=video.videoHeight) {
  ...
  const crop={sx:Math.round(sx),sy:Math.round(sy),sw:Math.round(sw),sh:Math.round(sh)};return {crop,videoRect:rectData(videoRect),roiRect:rectData(roiRect),videoWidth:vw,videoHeight:vh,scale,renderedWidth,renderedHeight,offsetX,offsetY,videoTransform:readVideoTransform(video),viewport:{width:Number(globalThis.innerWidth||0),height:Number(globalThis.innerHeight||0),visualWidth:Number(globalThis.visualViewport?.width||0),visualHeight:Number(globalThis.visualViewport?.height||0),orientation:globalThis.innerWidth>globalThis.innerHeight?'landscape':'portrait'}};
}
...
function measureCanvasSharpness(canvas){if(!canvas?.width||!canvas?.height)return 0;const data=canvas.getContext('2d',{willReadFrequently:true}).getImageData(0,0,canvas.width,canvas.height).data,gray=new Uint8Array(canvas.width*canvas.height);for(let p=0,i=0;p<gray.length;p++,i+=4)gray[p]=Math.round(data[i]*.299+data[i+1]*.587+data[i+2]*.114);return laplacianVariance(gray,canvas.width,canvas.height);}
