...
export class MarketWatchController {
  ...
  topValueCards(){return this.data.items.filter(item=>item.sources.includes('owned')&&item.ownedQuantity>0&&item.referencePrice!=null).sort((a,b)=>(b.referencePrice||0)-(a.referencePrice||0)).slice(0,5);}
  view(){const summary=portfolioSummary(this.data.items),items=sortItems(this.data.items.filter(item=>item.sources.includes(this.tab)),this.sort),unresolved=this.tab==='deck'?this.data.deckUnresolved:[],marketDecks=this.marketDecks(),hasSnapshots=this.data.items.some(item=>item.referencePrice!=null);
    const heroCards=this.topValueCards(),heroCard=heroCards.length?heroCards[this.heroCardIndex%heroCards.length]:null,heroArt=heroCard?preferredDeckArtwork(heroCard):'';
    return `<section class="page-stack market-page"><header class="market-hero">
        <h1 class="market-hero-eyebrow">Market Watch</h1>
        <div class="market-hero-spin" aria-hidden="true"><span class="market-hero-glow"></span><span class="market-hero-card"><span class="face front"${heroArt?` style="--hero-card-image:url(&quot;${esc(heroArt)}&quot;)"`:''}></span><span class="face back">${icon('chart')}</span></span></div>
