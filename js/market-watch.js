...
export class MarketWatchController {
  ...
  async loadRowHistory(printingId){
    ...
    // Con decine di righe che caricano lo storico man mano che scorrono in
    // vista, un onRender() qui ricostruiva l'intera pagina (tutte le righe,
    // gli osservatori, gli sparkline) una volta per riga — misurato: ~58
    // render completi scorrendo 60 carte, oltre 1,5s di lavoro solo JS/DOM.
    // Basta aggiornare lo sparkline della riga interessata.
    finally{this.rowHistoryLoading.delete(printingId);this.patchRowSparkline(printingId);}
  }
  patchRowSparkline(printingId){
