...
export class MarketWatchController {
  ...
  view(){const summary=portfolioSummary(this.data.items),items=sortItems(this.data.items.filter(item=>item.sources.includes(this.tab)),this.sort),unresolved=this.tab==='deck'?this.data.deckUnresolved:[],marketDecks=this.marketDecks(),hasSnapshots=this.data.items.some(item=>item.referencePrice!=null),confirmQueue=this.rarityMismatchQueue();
    return `<section class="page-stack market-page"><header class="market-hero">
      ...
      ${confirmQueue.length?`<div class="connection-banner warn"><span>${confirmQueue.length} cart${confirmQueue.length===1?'a ha':'e hanno'} una rarità da confermare su Cardmarket</span><button class="btn secondary small" data-market-confirm-queue-open>Conferma rarità</button></div>`:''}
      ...
      </section>${this.selected?this.detailView():''}${this.selectedDeck?this.deckDetailView(marketDecks):''}${this.confirmQueueOpen?this.confirmQueueView(confirmQueue):''}</section>`;}
  confirmQueueView(queue){return `<div class="detail-backdrop" data-market-confirm-queue-close><aside class="card-detail market-confirm-queue" role="dialog" aria-modal="true">
    <header class="market-confirm-queue-head"><button type="button" class="detail-close" data-market-confirm-queue-close aria-label="Chiudi">${icon('arrow')}</button><div><span class="eyebrow">Market Watch</span><h2>Conferma rarità</h2><p>${queue.length} cart${queue.length===1?'a ha':'e hanno'} più varianti su Cardmarket: conferma quale corrisponde alla tua per usarne il prezzo reale.</p></div></header>
    <div class="market-confirm-queue-list">${queue.length?queue.map(confirmQueueRow).join(''):'<div class="empty">Nessuna carta da confermare al momento.</div>'}</div>
  </aside></div>`;}
...
const MAPPING_STATUS_LABELS={PROVIDER_AGGREGATE:'Prezzo aggregato Cardmarket',AMBIGUOUS:'Ambiguo — verifica manuale consigliata',UNRESOLVED:'Non risolto',UNSUPPORTED:'Rarità non gestita',EXACT:'Corrispondenza esatta',MANUAL_OVERRIDE:'Confermato manualmente',
