...
export function collectionDetailView(id, scope, collection, connected, currentUser = '', marketItems = [], cardType = '') {
  ...
  return '<div class="detail-backdrop inventory-detail-backdrop" data-close-collection-detail><aside class="card-detail inventory-detail" role="dialog" aria-modal="true" aria-labelledby="collection-detail-title">'
    ...
    <div class="detail-layout"><div class="inventory-art-stage${typeBackground?' has-type-background':''}"${typeBackground?` style="background:linear-gradient(#090d1733,#090d1766) center/cover, url('${esc(typeBackground)}') center/contain no-repeat"`:''}><div class="detail-art">${item.imageUrl?`<img src="${esc(item.imageUrl)}" alt="${esc(item.cardName)}">`:icon('card')}</div></div>
