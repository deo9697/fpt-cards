...
export function renderDeckBoxCard(deck,options={}){
  const mode=options.mode==='market'?'market':'gallery',model=deckBoxModel(deck,options),action=mode==='market'?'data-market-deck':'data-deck-open';
  return `<button class="deck-box-card dynamic-deck-box ${mode==='market'?'market-deck-box':''} ${options.selected?'selected':''} ${options.active?'active':''}" ${themeAttributes(model)} ${action}="${esc(model.deckId)}" aria-label="${mode==='market'?'Apri il valore di':'Apri il mazzo'} ${esc(model.deckName)}"><span class="deck-box-visual ${model.template!=='procedural'?'uses-template':''}">${renderArtwork(model)}<i></i><b>F.P.T</b></span><span class="deck-box-copy"><strong>${esc(model.deckName)}</strong><small><b>${model.mainCount}</b> Main <i>•</i> <b>${model.extraCount}</b> Extra <i>•</i> <b>${model.sideCount}</b> Side</small>${mode==='market'?marketMeta(model):availabilityMeta(model)}</span></button>`;
}
